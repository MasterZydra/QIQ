...
func registerPredefinedConstants(environment *Environment) {
	...
	environment.AddPredefinedConstants("DIRECTORY_SEPARATOR", values.NewStr(os.DIR_SEP))
	environment.AddPredefinedConstants("FALSE", values.NewBool(false))
	environment.AddPredefinedConstants("TRUE", values.NewBool(true))
	environment.AddPredefinedConstants("NULL", values.NewNull())
	environment.AddPredefinedConstants("PHP_INT_MAX", values.NewInt(math.MaxInt64))
	environment.AddPredefinedConstants("PHP_INT_MIN", values.NewInt(math.MinInt64))
	environment.AddPredefinedConstants("PHP_INT_SIZE", values.NewInt(64/8))
	environment.AddPredefinedConstants("PHP_OS", values.NewStr(os.Os()))
	environment.AddPredefinedConstants("PHP_OS_FAMILY", values.NewStr(os.OS_FAMILY))
	environment.AddPredefinedConstants("PHP_EOL", values.NewStr(os.EOL))
	environment.AddPredefinedConstants("PHP_VERSION", values.NewStr(config.Version))
	environment.AddPredefinedConstants("PHP_MAJOR_VERSION", values.NewInt(config.MajorVersion))
	environment.AddPredefinedConstants("PHP_MINOR_VERSION", values.NewInt(config.MinorVersion))
